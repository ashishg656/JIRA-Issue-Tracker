 ...
 */
public class SelectStatusesFragment extends BaseFragment implements View.OnClickListener, AppRequestListener {

    RecyclerView recyclerView;
    LinearLayoutManager layoutManager;
    SelectStatusesFragmentListAdapter adapter;
    TextView done;
    ...
    public static SelectStatusesFragment newInstance(Bundle b) {
        SelectStatusesFragment frg = new SelectStatusesFragment();
        frg.setArguments(b);
        return frg;
    }
    ...
    private void loadData() {
        requestUrl = ZPreferences.getBaseUrl(getActivity()) + AppUrls.getAllStatusesUrl();

        AppRequests.makeGetAllStatusesRequest(requestUrl, this, getActivity());
    }

    @Override
    public void onRequestStarted(String requestTag) {
        if (requestTag.equalsIgnoreCase(RequestTags.GET_ALL_STATUSES)) {
            showProgressLayout();
            hideErrorLayout();
        }
    }
    ...
    private void setAdapterData(List<Status> object) {
        adapter = new SelectStatusesFragmentListAdapter(getActivity(), object, issueinterface.getSelectedStatus());
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.cancel_fragment:
                try {
                    issueinterface.setFilterDataAgain(AppConstants.FILTER_SET_STATUS, adapter.getSelectedItemsString());
